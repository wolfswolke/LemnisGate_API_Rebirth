...
def ip_handler(state):
    _get_remote_ip(state)
...
def date_time_handler():
    # 2024-09-09T22:00:23.197Z
    date = datetime.datetime.now().replace(microsecond=3).isoformat()
    return f"{date}Z"

def get_expiry_date_time():
    current_date = date_time_handler()
    parsed_date=datetime.datetime.strptime(current_date, "%Y-%m-%dT%H:%M:%S.%fZ")
    updated_date = parsed_date+ datetime.timedelta(hours=24)
    return updated_date
...
class Session_Manager:
    ...
    def create_session(self, user_id):
        ...
        # AAAAAAAAAAAAAAAA-114DBB6D73071B8E-BBBBBBBBBBBB-FFFB-8DCCEB5273D947A-fzGorG+FcCoaYjXzyE5CoJbRr3V0jf9uRWkfBL4n1yk=
        #
        # PlayFabId AAAAAAAAAAAAAAAA
        # PublisherId 114DBB6D73071B8E
        # Enity ID BBBBBBBBBBBB
        # Title ID FFFB
        user_token = f"AAAAAAAAAAAAAAAA-114DBB6D73071B8E-BBBBBBBBBBBB-FFFB-8DCC{rand_id}"

        expires = time.time() + 2629743
    ...
    def get_session_content(self, session_id):
        ...
        ret = {
            ...
            "EntityId": self.sessions[session_id]["EntityId"]
        }
        return ret

    def extend_session(self, session_id):
        try:
            self.sessions[session_id]["expires"] = time.time() + 2629743
            self.save_sessions()
        except KeyError:
            print(f"Session ID: {session_id} not found.")
            return None
